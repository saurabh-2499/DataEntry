...
public partial class Mutation_reject : System.Web.UI.Page
{
    ...
    DataRow Drow;
    string page_name = "समस्या निराकरण";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                Session["page_heading"] = "समस्या निराकरण करणे";
                txtMutNo.Focus();
                btnMutNo.Enabled = true;
            }
    ...
    protected void btnMutNo_Click(object sender, EventArgs e)
    {
        try
        {
            int date_cnt = obj.funReturnSingleValueInt(Convert.ToString(Session["DataBaseName"]), Session["SchemaName"].ToString() + ".edit_holder_detail_kp ", "count(*)", "ccode='" + Convert.ToString(Session["ccode"]) + "' and khata_master_declaration = false and khata_master_declair_date  isnull  ", "");
            ...
            {
                if (txtMutNo.Text.Trim() != String.Empty)
                {
                    DataTable dtKhataNoInfo = new DataTable();
                    int ehd_cnt = con.funReturnSingleValueInt(Convert.ToString(Session["DataBaseName"]), Session["SchemaName"].ToString() + ".edit_holder_detail_kp", "count(*)", "ccode='" + Convert.ToString(Session["ccode"]) + "' and trim(khata_no)='" + txtMutNo.Text.Trim() + "'", "");
                    if (ehd_cnt > 0)
                    {
                        dtKhataNoInfo = obj.funReturnDataTable(Convert.ToString(Session["DataBaseName"]), Session["SchemaName"].ToString() + ".edit_holder_detail_kp e ," + Session["SchemaName"].ToString() + ".m_khata_type m ", "fname,mname,lname,topan_name,e.khata_type,m.khata_description", "ccode='" + Convert.ToString(Session["ccode"]) + "' and trim(khata_no)='" + txtMutNo.Text.Trim() + "' and m.khata_type=e.khata_type and  edit_flag<>'34'  and edit_flag<>'40' ", "old_mut_no,usrno");
                    }
                    else
                    {
                        dtKhataNoInfo = obj.funReturnDataTable(Convert.ToString(Session["DataBaseName"]), Session["SchemaName"].ToString() + ".holder_detail h ," + Session["SchemaName"].ToString() + ".m_khata_type m ", "fname,mname,lname,topan_name,h.khata_type,m.khata_description", "ccode='" + Convert.ToString(Session["ccode"]) + "' and trim(khata_no)='" + txtMutNo.Text.Trim() + "' and m.khata_type=h.khata_type", "old_mut_no,usrno");
                    }
                    if (dtKhataNoInfo.Rows.Count > 0)
                    {
                        string KhataNoPending = string.Empty;
                        KhataNoPending = con.KhataNoPendingChecks(Convert.ToString(Session["DataBaseName"]), Session["SchemaName"].ToString(), Convert.ToString(Session["ccode"]), "1", txtMutNo.Text.ToString().Trim());
                        ...
                        Session["khata_no"] = txtMutNo.Text;
                        btnMutNo.Enabled = false;
                        ...
                        lblSelectedkhataNoDisp.Text = txtMutNo.Text;
    ...
    public Boolean prev()
    {
        try
        {
            DataTable dseditf7k_spell_correction = obj.funReturnDataTable((string)Session["DataBaseName"].ToString(), Session["SchemaName"].ToString() + ".edit_form7_khata_audit", "distinct  khata_no ,fname ,mname,lname,topan_name ,operation ,edit_flag,sr_no", "ccode='" + (string)Session["ccode"] + "' and trim(khata_no)= '" + txtMutNo.Text.ToString().Trim() + "' and edit_flag  in ('31','33') and operation in ('OLD','NEW' )  and  (pin,pin1,pin2,pin3,pin4,pin5,pin6,pin7,pin8) in (select pin,pin1,pin2,pin3,pin4,pin5,pin6,pin7,pin8   from " + Session["SchemaName"].ToString() + ".edit_mut where ccode='" + (string)Session["ccode"] + "' and  marked_flag='Edit' and confirm_flag<>'Confirm' and approve_flag='Approve')", "sr_no");
    ...
    public Boolean spellCorrection()
    {
        try
        {
        DataTable dseditf7k_spell_correction = obj.funReturnDataTable((string)Session["DataBaseName"].ToString(), Session["SchemaName"].ToString() + ".edit_form7_khata_audit", "distinct  khata_no ,fname ,mname,lname,topan_name ,operation ,edit_flag,sr_no,'' as newfname , '' as newmname ,''as newlname,'' as newtopan_name", "ccode='" + (string)Session["ccode"] + "' and trim(khata_no) = '"+ txtMutNo.Text.ToString().Trim()+"' and edit_flag  in ('31','33') and operation in ('OLD','NEW' )  and  (pin,pin1,pin2,pin3,pin4,pin5,pin6,pin7,pin8) in (select pin,pin1,pin2,pin3,pin4,pin5,pin6,pin7,pin8   from " + Session["SchemaName"].ToString() + ".edit_mut where ccode='" + (string)Session["ccode"] + "' and  marked_flag='Edit' and confirm_flag<>'Confirm' and approve_flag='Approve')", "sr_no");
        if (dseditf7k_spell_correction != null && dseditf7k_spell_correction.Rows.Count > 0)
        {
            DataTable dt_spell = dseditf7k_spell_correction.Clone();
            if (dseditf7k_spell_correction != null && dseditf7k_spell_correction.Rows.Count > 0)
    ...
    protected void lnkKhataNoDeletionSurveyWise_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Convert.ToString(txtMutNo.Text) == "")
            {
